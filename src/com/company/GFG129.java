...
class Solution{
   ...
   long count(long n)
    ...
    public static long f(int[] num,int idx,int ct,int turn,String s,Long[][][] dp) {
        if(idx>=num.length) {
            if(ct!=0) return 0;
            return 1;
        }
        ...
        long ans;
        if(num[idx]==1) {
            ans = f(num,idx+1,ct-1,turn,s+"1",dp)+f(num,idx+1,ct,1,s+"0",dp);
        } else {
            if(turn==0) {
                ans = f(num,idx+1,ct,turn,s+"0",dp);
            } else {
                ans = f(num,idx+1,ct-1,turn,s+"1",dp)+f(num,idx+1,ct,1,s+"0",dp);
            }
        }
